...
namespace NewRecap.Pages.AdminPages
{
    ...
    public class EditEmployeeLNameModel : PageModel
    {
        ...
        {
            var lastName = (Employees.EmployeeLName ?? string.Empty).Trim();
            const int dbMaxName = 50;

            if (lastName.Length > dbMaxName)
            {
                ModelState.AddModelError("Employees.EmployeeLName", "Username must be at most 50 characters.");
            }
            if (string.IsNullOrWhiteSpace(lastName))
            {
                ModelState.AddModelError("Employees.EmployeeLName", "Username must be more than 0 characters.");
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    // Query for update
                    string cmdText = "UPDATE Employee SET EmployeeLName = @EmployeeLName WHERE EmployeeID = @EmployeeID";
                    // Create the command.
        ...
        private void PopulateEmployeeList(int id)
        {
            using (SqlConnection conn = new SqlConnection(AppHelper.GetDBConnectionString()))
            {
                string query = "SELECT EmployeeID, EmployeeLName FROM Employee WHERE EmployeeID = @EmployeeID";
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        Employees = new EmployeeView
                        {
                            ...
                            EmployeeLName = reader.GetString(1)
                        };
                    }
                }
            }
