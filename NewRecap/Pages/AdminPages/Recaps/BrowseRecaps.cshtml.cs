...
namespace NewRecap.Pages.AdminPages.Recaps
{
    public class BrowseRecapsModel : PageModel
    {
            ...
            using (SqlConnection conn = new SqlConnection(AppHelper.GetDBConnectionString()))
            {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            RecapView recap = new RecapView
                            {
                                ...
                                RecapWDYD = reader.GetString(3),
                                RecapWLTD = reader.GetString(4),

                                RecapAssetNumber = reader.IsDBNull(5) ? null : reader.GetInt32(5),
                                RecapSerialNumber = reader.IsDBNull(6) ? string.Empty : reader.GetString(6),

                                IP = reader.IsDBNull(7) ? string.Empty : reader.GetString(7),
                                WAM = reader.IsDBNull(8) ? string.Empty : reader.GetString(8),
                                Hostname = reader.IsDBNull(9) ? string.Empty : reader.GetString(9),
                                RecapTicketNumber = reader.GetString(10),
                                CrossCheckedBy = GetEmployeeNameById(reader.GetInt32(11)),
                            ...
                            recap.Segments = PopulateHardwareRecapSegments(reader.GetInt32(0));
                            Recaps.Add(recap);
                ...
                else
                {
                    ...
                    SELECT
                      ...
                      hr.HardwareRecapWDYD,
                      hr.HardwareRecapWLTD,
            ...
            using (SqlConnection conn = new SqlConnection(AppHelper.GetDBConnectionString()))
            {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            RecapView recap = new RecapView
                            {
                                ...
                                StartingMileage = reader.IsDBNull(7) ? null : reader.GetInt32(7),
                                EndingMileage = reader.IsDBNull(8) ? null : reader.GetInt32(8),
                                Customer = reader.IsDBNull(9) ? string.Empty : reader.GetString(9),
                                RecapCredsSetUsed = reader.IsDBNull(10) ? string.Empty : reader.GetString(10),
                                RecapInventoryUsed = reader.IsDBNull(11) ? string.Empty : reader.GetString(11),
                                RecapInventoryLeft = reader.IsDBNull(12) ? string.Empty : reader.GetString(12),
                            ...
                            recap.Segments = PopulateRecapSegments(reader.GetInt32(0));
                            Recaps.Add(recap);
                        }
                    }
                }
                else
                {
                    ...
                    SELECT
                      ...
                      r.Customer,
                      r.RecapCredsSetUsed,
                      r.RecapInventoryUsed,
                      r.RecapInventoryLeft
        ...
        private string PopulateRecapSegments(int recapID)
        {
            ...
            var allSegments = new List<RecapSegment>();

            static DateTime? GetNullableDateTime(SqlDataReader r, int i)
                => r.IsDBNull(i) ? null : r.GetDateTime(i);
        ...
        WHERE RecapID = @RecapID;", conn))
            {
                ...
                while (reader.Read())
                {
                    DateTime sDate = reader.GetDateTime(0);
                    DateTime sTime = reader.GetDateTime(1);
                    DateTime eDate = reader.GetDateTime(2);
                    DateTime eTime = reader.GetDateTime(3);

                    allSegments.Add(new RecapSegment
                    {
                        SegmentType = "Work",
                        StartDate = sDate,
                        StartTime = sTime,
                        EndDate = eDate,
                        EndTime = eTime,
                        SortStart = CombineDateAndTime(sDate, sTime)
                    });
                }
            }
        ...
        WHERE RecapID = @RecapID;", conn))
            {
                ...
                while (reader.Read())
                {
                    var sDate = GetNullableDateTime(reader, 0);
                    var sTime = GetNullableDateTime(reader, 1);
                    var eDate = GetNullableDateTime(reader, 2);
                    var eTime = GetNullableDateTime(reader, 3);
        ...
        WHERE RecapID = @RecapID;", conn))
            ...
            string fmtSeg(RecapSegment seg)
                => $"{seg.SegmentType} {combine(seg.StartDate, seg.StartTime)} → {combine(seg.EndDate, seg.EndTime)}";

            var orderedSegments = allSegments
                .OrderBy(s => s.SortStart ?? DateTime.MaxValue)
                .ToList();
            ...
            {
                parts.Add($"Segment {i + 1}: {fmtSeg(orderedSegments[i])}");
            }

            return parts.Count == 0 ? "" : string.Join("<br></br>", parts);
        }

        private string PopulateHardwareRecapSegments(int recapID)
        {
            using var conn = new SqlConnection(AppHelper.GetDBConnectionString());
            conn.Open();
