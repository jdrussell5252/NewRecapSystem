...
namespace NewRecap.Pages.AdminPages.Recaps
{
    public class RecapWeeklyLogModel : PageModel
    {
        ...
        {
            var reports = GetWeeklyEmployees(day, 1); // reuse, still works for single day
        ...
        private List<WeeklyReport> GetWeeklyEmployees(DateTime weekStart, int spanDays = 7)
        {
            ...
            string sql = @"
                ...
                (
                    ...
                    JOIN Employee e ON ehr.EmployeeID = e.EmployeeID
                ) Combined
                WHERE ActivityDate >= @WeekStart
                  AND ActivityDate < DATEADD(DAY, @SpanDays, @WeekStart)
            ...
            cmd.Parameters.Add("@SpanDays", SqlDbType.Int).Value = spanDays;
